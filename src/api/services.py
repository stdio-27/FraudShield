...
class ModelManager:
    ...
    # Hardcoded training set parameters for Amount Z-score (approximated for creditcard dataset)
    AMOUNT_MEAN = 88.3496
    AMOUNT_STD = 250.1201
    ...
    def _engineer_features(self, req_data: dict) -> pd.DataFrame:
    ...
    def predict(self, req_data: dict, threshold: float = 0.5) -> Tuple[float, bool, str, Any]:
        ...
        df_engineered = self._engineer_features(req_data)
        ...
        if is_flagged:
            ...
            # xgboost binary classification often returns a single array or list of arrays depending on shap version
            if isinstance(shap_vals, list):
                shap_vals = shap_vals[1]  # positive class
            
            # The output for a single sample is an array of feature impacts
